    ...
    <div class="container" v-loading.fullscreen.lock="!show" element-loading-text="拼命加载中" element-loading-background="rgba(255, 255, 255, 0.5)">
        ...
        <div class="leave-content">
                ...
                <li v-for="vo in list">
                    ...
                    <div class="l content">
                        ...
                        <div class="reply" v-for="zo in vo.list">
                            <a :href="zo.url" target="_blank">
                                <img :src="zo.head" />
                            </a>
                            <h4>
                                <a :href="zo.url" target="_blank">{{zo.name}}</a>
                                回复:
                            ...
                            <span>
                                {{zo.time}}
                                <i class="iconfont">&#xe61a;</i>
                                {{zo.region || "未知地点"}} {{zo.os}} {{zo.browser}}
                                <a class="r" @click="pid=zo.id;pcl='回复'+(zo.name || '匿名')+':'" href="#">回复</a>
                            </span>
                            <div class="reply" v-for="zzo in zo.list">
                                <a :href="zzo.url" target="_blank">
                                    <img :src="zzo.head" />
                                </a>
                                <h4>
                                    <a :href="zzo.url" target="_blank">{{zzo.name}}</a>
                                    回复:
                                    <a :href="zo.url" target="_blank">{{zo.name}}</a>
                                </h4>
                                <p v-html="zzo.content"></p>
                                <span>{{zzo.time}}
                                    <i class="iconfont">&#xe61a;</i>
                                    {{zzo.region || "未知地点"}} {{zzo.os}} {{zzo.browser}}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="hot l">
                <div class="title">
                    <h1 class="l">人气文章</h1>
                    <router-link class="r" to="/">>更多</router-link>
                </div>
                <div class="content">
                    <ul>
                        <li v-for="vo,i in init.article_a">
                            <router-link :to="'/content/'+vo.id">{{vo.title}}</router-link>
                            <span class="r">{{vo.a}}</span>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="hot l">
                ...
                <div class="content">
                    <ul>
                        <li v-for="vo,i in init.article_z">
                            <router-link :to="'/content/'+vo.id">{{vo.title}}</router-link>
                            <span class="r">{{vo.z}}</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="page" v-show="list.length>0">
            <el-pagination background :page-size="page.size" @current-change="getPage"
                           layout="prev, pager, next"  :total="page.count">
            </el-pagination>
        </div>
    </div>
	</el-container>
...
<script>
	export default {
		...
		methods: {
            ...
            isUser:function () {
                if(this.user.name.length>0 && this.user.qq.length>0
                    &&  this.user.email.length>0 && this.user.url.length>0){
                  return true;
                }else{
                  return false;
                }
